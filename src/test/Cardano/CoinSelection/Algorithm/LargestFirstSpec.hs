...
spec = do
    ...
    describe "Coin selection: largest-first algorithm: properties" $ do
        it
            "forall (UTxO, NonEmpty TxOut), for all selected input, there's no \
...
propInputDecreasingOrder (CoinSelectionData utxo txOuts) =
    isRight selection
        ==> let Right (CoinSelectionResult s _) = selection
             in prop s
  where
    prop (CoinSelection inps _ _) =
        let
            utxo' =
                (Map.toList . unCoinMap) $
                    utxo
                        `excluding` Set.fromList (entryKey <$> coinMapToList inps)
         in
            unless (L.null utxo') $
                (L.minimum (entryValue <$> coinMapToList inps))
                    `shouldSatisfy` (>= (L.maximum (snd <$> utxo')))
    selection =
        runIdentity $
            runExceptT $
...
propSelectionMinimal (CoinSelectionData inpsAvailable outsRequested) =
    isRight result
        ==> let Right (CoinSelectionResult selection _) = result
             in prop selection
  where
    ...
    prop (CoinSelection inpsSelected _ changeGenerated) =
        cover
            ...
            $ cover
                ...
                (amountSelected == amountRequired)
                "amountSelected = amountRequired"
            $ if amountSelected > amountRequired
                then
                    changeGenerated
                        `shouldBe` [amountSelected `C.distance` amountRequired]
                else changeGenerated `shouldSatisfy` null
      where
