...
spec = do
    describe "CoinMap properties" $ do
        it "CoinMap coverage is adequate" $
            checkCoverage $
                prop_CoinMap_coverage @Int
        it "CoinMapEntry coverage is adequate" $
            checkCoverage $
                prop_CoinMapEntry_coverage @Int
        ...
        it "coinMapFromList preserves total value" $
            checkCoverage $
                prop_coinMapFromList_preservesTotalValue @Int
        it "coinMapToList preserves total value" $
            checkCoverage $
                prop_coinMapToList_preservesTotalValue @Int
        it "coinMapFromList . coinMapToList = id" $
            checkCoverage $
                prop_coinMapToList_coinMapFromList @Int
        it "coinMapToList . coinMapFromList = id (when no keys duplicated)" $
            checkCoverage $
                prop_coinMapFromList_coinMapToList @Int
    ...
    describe "CoinSelection properties" $ do
        it "monoidal append preserves keys" $
            checkCoverage $
                prop_coinSelection_mappendPreservesKeys @Int @Int
...
prop_CoinMap_coverage m =
    property
        ...
        $ cover
            10
            (1 == length (Set.fromList $ entryValue <$> coinMapToList m))
...
prop_CoinMapEntry_coverage entries =
    property
        $ cover
            2
            (null entries)
        ...
        $ cover
            2
            (not (null entries) && length uniqueKeys == 1)
            "coin map entry list has one unique key"
        $ cover
            2
            (not (null entries) && length uniqueKeys == 2)
            "coin map entry list has two unique keys"
        $ cover
            2
            (not (null entries) && length uniqueKeys >= 3)
            "coin map entry list has multiple unique keys"
        $ cover
            2
            (not (null entries) && null duplicateKeys)
            "coin map entry list has no duplicate keys"
        $ cover
            2
            (not (null entries) && length duplicateKeys == 1)
            "coin map entry list has one duplicate key"
        $ cover
            2
            (not (null entries) && length duplicateKeys == 2)
            "coin map entry list has two duplicate keys"
        $ cover
            2
            (not (null entries) && length duplicateKeys >= 3)
...
prop_coinMapFromList_preservesTotalValueForEachUniqueKey ::
    ...
    [CoinMapEntry u] ->
    Property
prop_coinMapFromList_preservesTotalValueForEachUniqueKey entries =
    property $
        mkEntryMap entries
            `shouldBe` mkEntryMap (coinMapToList (coinMapFromList entries))
  where
    mkEntryMap =
...
prop_coinMapFromList_preservesTotalValue ::
    ...
    [CoinMapEntry u] ->
    Property
prop_coinMapFromList_preservesTotalValue entries =
    property $
...
prop_coinMapToList_preservesTotalValue ::
    CoinMap u ->
    Property
prop_coinMapToList_preservesTotalValue m =
    property $
...
prop_coinMapToList_coinMapFromList ::
    ...
    CoinMap u ->
    Property
prop_coinMapToList_coinMapFromList m =
    property $
...
prop_coinSelection_mappendPreservesKeys s1 s2 = property $ do
    Map.keysSet (unCoinMap $ inputs $ s1 <> s2)
        `shouldBe` Map.keysSet (unCoinMap $ inputs s1)
            `Set.union` Map.keysSet (unCoinMap $ inputs s2)
    Map.keysSet (unCoinMap $ outputs $ s1 <> s2)
        `shouldBe` Map.keysSet (unCoinMap $ outputs s1)
            `Set.union` Map.keysSet (unCoinMap $ outputs s2)
...
prop_CoinSelectionData_coverage ::
    ...
    Property
prop_CoinSelectionData_coverage (CoinSelectionData inps outs) =
    property
        $ cover
            90
...
prop_algorithm_inputsAvailable_outputsRequested ::
    ...
    Expectation
prop_algorithm_inputsAvailable_outputsRequested =
    \(CoinSelectionData inputsAvailable outputsRequested) ->
        const $
            coinMapValue inputsAvailable
                `shouldSatisfy` (>= coinMapValue outputsRequested)
...
prop_algorithm_inputsSelected_outputsSelected_change = const $
    \(CoinSelectionResult (CoinSelection{inputs, outputs, change}) _) ->
        coinMapValue inputs
            `shouldBe` (coinMapValue outputs `C.add` mconcat change)
...
prop_algorithm algorithm verifyExpectation params =
    withMaxSuccess 1_000 $ monadicIO $ QC.run $ do
        ...
        pure $
            isRight mResult
                ==> let Right result = mResult
                     in verifyExpectation params result
  where
...
instance (Buildable i, Buildable o) => Buildable (CoinSelectionData i o) where
    build (CoinSelectionData inps outs) =
        mempty
...
instance
    (Arbitrary i, Arbitrary o, Ord i, Ord o) =>
...
instance
    ...
    where
    shrink (CoinSelectionData inps outs) =
        uncurry CoinSelectionData
            <$> zip
                (shrink inps)
